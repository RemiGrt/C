...
void initPattePB5_Bouton(void);
void initPattePB3_out(void);
...
/*===============================================================================
  FUNCTION:     void initPattePB5_Bouton_Interruption(void)
  DESCRIPTION:  Configuration de la patte PB5 (PORT B, GPIOB) en mode entrée simple
				+ activation demande interruption si front descendant
				+ démasquage de la demande au niveau du NVIC pour que ce CPU soit sensible
				  a la demande
  PARAMETERS:   rien
...
void initPattePB5_Bouton(void)
{
    //1. Activer horloge du GPIOx concernée : ici GPIOB
    RCC->AHBENR |=(1<<18);     // Activation horloge du GPIOB
  //2. Configurer la patte en mode INPUT 
    GPIOC->MODER &=~ (1<<11);   // choix  Input mode bit b11 à '0'
    GPIOC->MODER &=~(1<<10);   // choix  Input mode bit b10 à '0'
  //3.désactiver les résistances de tirages 
    GPIOC->PUPDR &=~(1<<11);   // desactivation resistance de tirage: bit b11 à '0'
    GPIOC->PUPDR &=~(1<<10);   // desactivation resistance de tirage: bit b10 à '0'
}
