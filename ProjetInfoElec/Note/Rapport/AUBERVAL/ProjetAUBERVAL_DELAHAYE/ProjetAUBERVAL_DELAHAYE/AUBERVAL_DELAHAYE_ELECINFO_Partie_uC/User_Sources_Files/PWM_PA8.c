...
/*----------------------------------------------------------------------------
TIM1_IRQHandler (void) : Notez que cette n'est pas appélée dans main
						Mais est executée par déclenchement matérielle
	Routine interruption du Timer TIM1 : vecteur interruption TIM1_IRQn
	Cette fonction est appelee/declenchee a chaque fois que:
	1. demande interruption suite a un evenement debordement
	2. demande interruption suite a un evenement changement dans les bits de controle Capture/compare
	3. demande interruption suite a un evenement Trigger
	Ici nous somme concerné par le 1.
   ...
   if((TIM1->SR & (1<<0))== (1<<0)) // vérifions que c'est bien le debordement qui demande interruption
    {
        TIM1->SR &= ~(1<<0);  		// reset du drapeau de demande interruption
				TIM1->CCR1 += 1;         // augmenter le rapport cyclique de 1
				if(TIM1->CCR1 == 199)   // si le rapport cyclique est maximum
				{
					TIM1->CCR1 = 99;        // remettre à zero
				}
