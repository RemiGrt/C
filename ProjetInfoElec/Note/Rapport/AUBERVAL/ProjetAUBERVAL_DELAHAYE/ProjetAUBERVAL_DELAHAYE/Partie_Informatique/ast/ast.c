...
/*fextern etapeNode *findEtapeByName(etapeNode *head,char *s){
	etapeNode* recherche=head;
	while (recherche!=NULL){
		if strcmp(recherche->name,s)==0){
			return recherche;
		}
		head=head->next;
	}
...
void generateCode(etapeNode *en, transitionNode *c)
{
  transitionNode *transition;
  etapeNode *etape;
  actionNode *an;
  ...
  while(etape->id <3){
  printf("# test de franchissement de la transition t%d\n",etape->id);
  
    printf("\tpush\t%d\n",14+etape->id);
    printf("\tpushi\t1\n");
    printf("\teq\n");
    printf("\tpush\t%d\n",1+transition->id);
    printf("\tpush\t%d\n",6); //Il n'y a pas d'ID pour action (ex:an0)
    printf("\tpush\t%d\n",128); // La valeur mediane du potentiomètre 
    char s[10];
    if(etape->id%2==0){ 
		strcpy(s, "ls");
	}else{
