/*! \file ast.c
 * Le fichier ast.c permet de générer l'exécutable grafcet.out
 * C'est cette exécutable qui va permettre d'obtenir le code assembleur qu'on pourra ensuite injecté dans l'assembleur
 * En entré de l'éxécutable nous donnons le grafcet a analyser
 * Le principe va être d'interpréter le grafcet et de le traduire en un arbre qui sera ensuite analyser pour créer le code assembleur
*/
#include "ast.h"
...
/*!
 * La fonction createOperatorNode va creer les noeuds d'opérateur 
 */
...
void generateCode(etapeNode *en, transitionNode *tn)
{
  ...
  actionNode *an;
  int n,i;
  int jumpTransition = 0;
  int jumpAffectation = 0;
  ...
  while (etape != NULL)
	...
	while (transition != NULL)
	{
		printf ("#evaluation de la transition %s\n", transition->name);
		
		etapeRefNode *erefAmont, *erefAval;
		
		erefAmont = transition -> etapesAmont;
		while(erefAmont != NULL)
		{
			etape = erefAmont -> etape;
			printf("\tpush\t%d\n",14+etape->id);
			printf("\tpushi\t1\n");
			printf("\teq\n");
			
			erefAmont = erefAmont -> next;			
		}
		
		generateAsmExpression(transition -> receptivite,stdout);
		printf("\tand\n");
		
		printf("\tjf Jtransition%d\n",jumpTransition); 
		
		erefAval = transition -> etapesAval;
		while(erefAval != NULL)
		{
			etape = erefAval -> etape;
			printf("\tpushi\t1\n");
			printf("\tpop\t%d\n",14+2*n+etape->id);
			
			erefAval = erefAval -> next;
		}
  ...
  while (etape != NULL)
	...
	printf("\tjf suiteloop\n"); // si pas stable on jp a suiteloop 
	
	printf("\tpushi\t1\n");
	printf("\tpop\t%d\n",13);
	
	printf("suiteloop:\n"); // qui renvoie alors dans mainloop
	printf("\tjp mainloop\n");
	
	// Fin test de stabilité
	...
	while(etape != NULL)
	{
		...
		printf("\tpushi\t1\n");
		printf("\teq\n");
		printf("\tjf JAffectation%d\n",jumpAffectation); 

		printf("\tpushi\t1\n");
		printf("\tpop\t%d\n",8+etape->id);
		
		printf("JAffectation%d:\n",jumpAffectation);	
		jumpAffectation++;	
		etape = etape -> next;
	}
