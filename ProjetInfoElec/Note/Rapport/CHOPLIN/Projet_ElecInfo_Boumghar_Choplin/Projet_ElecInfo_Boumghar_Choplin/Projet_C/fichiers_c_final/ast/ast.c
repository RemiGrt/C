 ...
 * Ce programme est un compilateur, il travaille directement avec bison, lex et ast.h.
 * Il nous a été fournit en grande partie pour la réalisation de notre projet.
 * Il permet, à partir d'un grafcet de générer un code assembleur qui l'interprète.
 * Les fonctions maakeDual et generateCode sont appelés depuis y.tab.c
 * 
...
/*! \brief makeDual(etapeNode *en, transitionNode *tn)
// construit l'arbre syntaxique, en partant des structures définies dans ast.h.
*/
  ...
  while (transition!=NULL)
    {
      ...
      while (ern!=NULL)
        {
          ...
          etape = findEtapeByName(en,ern->name); //ToDO : appel de findEtapeByName, retour stocké dans etape
          ...
          if (etape!=NULL)
            {
              ern->etape=etape;
              pushRefTransition(&etape->transitionsAval,transition->name); // ToDo : appel de pushRefTransition
            }
          else
            {
          ...
          if (etape!=NULL)
            {
              ...
              ern->etape=etape;
              pushRefTransition(&etape->transitionsAmont,transition->name); // ToDo : appel de pushRefTransition
            }
          else
            {
      ...
      transition = transition->next;// ToDo : Mise à jour de transition avec la transition suivante
      ...
      while (trn!=NULL)
        {
          ...
          trn = trn->next; // ToDo  Mise à jour de trn avec trn suivant
        }
      ...
      while (trn!=NULL)
        {
          ...
          trn = trn->next; // ToDo meme chose que le derner while (3 instructions)
        }
...
/*! \brief La fonction generateCode(etapeNode *en, transitionNode *tn)
// exploite l'arbre réalisé dans makeDual afin de générer le code assembleur.
*/
