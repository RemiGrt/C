...
/*===============================================================================
  ...
  PARAMETERS: Variables et tableaux relatives à la machine virtuelle développée
  RETURNS: aucun
	...
	{	
		...
		{	
			...
			case I_PUSHI :
							...
							*PC += 1; // On incrémente *PC de 2
					if (DEBUG_TERMINAL)
					{
						sprintf(Retour_Code,"%d: %d\n",*PC,Code[*PC]);
			...
			// Instruction ADD
			case I_ADD :
							*SP = * SP - 1; // ON décrémente la valeur de *SP
							Stack[*SP] = Stack[*SP] + Stack[*SP+1]; // On additionne les deux valeurs sommets de la pile
							*PC = *PC + 1 ;// incrémentation du *PC : on passe a l'instruction suivante
					break;
			...
			// Instruction PUSH
		    	case I_PUSH :
							if (LINUX_MODE)
							{
								...
								*SP = * SP +  1; // on incrémente la valeur de *SP
			...
			// Instruction POP
		    	case I_POP :
							...
							*SP = *SP - 1 ; // On décrémente *SP
							*PC = * PC +  1; // On incrémente *PC de 1
			...
			// Instruction JUMP FALSE ( ne s'exécute qu'après une condition)
				case I_JF : 
							...
							if(Stack[*SP]== 0)
							{
									...
									*PC = Code[*PC+1]; // JUMP : *PC prend la valeur de l'instruction suivante ( qui corre*SPond à l'étape à laquelle on veut que le programme aille)
							}
							else
							{
									*PC = * PC +  2;
