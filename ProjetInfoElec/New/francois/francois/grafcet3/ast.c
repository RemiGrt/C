...
void generateAsmExpression(nodeType *n, FILE *fout)
{
        ...
        switch (n->type)
        {
				...
				else if (strncmp(n->t_identifier.ident,"but2",4)==0)
				{
					if (n->t_identifier.flag==0)
						fprintf(fout,"\tpush\t6\n");
					else if (n->t_identifier.flag==1)
...
// 16: e0...nbEtapes-1.
// 16+nbEtapes: old_e0...
// 16+nbEtapes*2: appel_e0...
// 16+nbEtapes*3: reponse_e0...
...
void generateCode(etapeNode *en, transitionNode *tn)
{
	...
	printf("# 006 : but2\n");
	printf("# 007 : but2'rise\n");
	printf("# 008 : but2'fall\n");
	printf("# 009 : an0\n");
	printf("# 010 : an1\n");
	printf("# 011 : led0\n");
	printf("# 012 : led1\n");
	...
	while (transition!=NULL)
	{
		...
		// La il y a encore du boulot: generer le bon nombre de and, puis la receptivvite
                printf("GenerateAsm debut\n");
                generateAsmExpression(transition->receptivite,stdout);
                printf("GenerateAsm fin\n");
